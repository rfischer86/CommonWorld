...
@Component({
  selector: 'app-form-file-element',
  templateUrl: './file.component.html',
  styleUrls: ['./file.component.scss']
})

export class FormularFileElementComponent implements OnInit, AfterViewInit, OnDestroy {
  @ViewChild('fileInput') fileInput: ElementRef<HTMLInputElement>;
  @ViewChild('fileLabel') fileLabel: ElementRef<HTMLInputElement>;
  ...
  ngAfterViewInit () {
    this.fileLabel.nativeElement.value = this.contentData.value ? this.contentData.value : null;
    if (this.parentId) {
      this.onFocusout();
    }
  }
  ...
  onFocusout() {
    ...
    this.contentData.value = this.fileLabel.nativeElement.value;
  ...
  triggerFileSelect(event: MouseEvent) {
  ...
  loadFile(files: FileList) {
    if (files && files.length !== 0) {
      this.fileLabel.nativeElement.value = files[0].name;
    }
  }
